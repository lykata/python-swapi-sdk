...
class Starship(UserDict):
    """This represents a JSON object.
    """
    class StarshipClassProperty(object):
    ...
    class HyperdriveRatingProperty(object):
        """ This class is a schema-validating wrapper around a string.
        """
    ...
    def __init__(self, data=None, **kwargs):
        """Initialization for the Starship object.
        It can be initialized with an object, or by passing each
        object property as a keyword argument.
        """
        new_data = {}
        try:
            prop = data["starship_class"] if ("starship_class" in data) else kwargs["starship_class"]
            if not isinstance(prop, self.StarshipClassProperty):
                new_data["starship_class"] = self.StarshipClassProperty(prop)
        except KeyError:
            raise ValueError("Missing property 'starship_class'")
        try:
            prop = data["hyperdrive_rating"] if ("hyperdrive_rating" in data) else kwargs["hyperdrive_rating"]
            if not isinstance(prop, self.HyperdriveRatingProperty):
                new_data["hyperdrive_rating"] = self.HyperdriveRatingProperty(prop)
        except KeyError:
            raise ValueError("Missing property 'hyperdrive_rating'")
        try:
        ...
        try:
            prop = data["MGLT"] if ("MGLT" in data) else kwargs["MGLT"]
            if not isinstance(prop, self.MGLTProperty):
                new_data["MGLT"] = self.MGLTProperty(prop)
        except KeyError:
            raise ValueError("Missing property 'MGLT'")
        try:
    ...
    def GetStarshipClass(self):
        return self.data["starship_class"]
    
    def SetStarshipClass(self, new_value):
        if not isinstance(new_value, self.StarshipClassProperty):
            self.data["starship_class"] = self.StarshipClassProperty(new_value)
        else:
            self.data["starship_class"] = new_value
        return self

    def GetHyperdriveRating(self):
        return self.data["hyperdrive_rating"]
    
    def SetHyperdriveRating(self, new_value):
        if not isinstance(new_value, self.HyperdriveRatingProperty):
            self.data["hyperdrive_rating"] = self.HyperdriveRatingProperty(new_value)
        else:
            self.data["hyperdrive_rating"] = new_value
        return self
    ...
    def GetMGLT(self):
        return self.data["MGLT"]
    
    def SetMGLT(self, new_value):
        if not isinstance(new_value, self.MGLTProperty):
            self.data["MGLT"] = self.MGLTProperty(new_value)
        else:
            self.data["MGLT"] = new_value
        return self
